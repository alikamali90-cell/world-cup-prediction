...
export default function SemiFinalPredictionPage() {
  ...
  const getSemiFinalLockTime = (matchList) => {
  ...
  const isSemiFinalLocked = (matchList) => {
    const lockTime = getSemiFinalLockTime(matchList);
  ...
  const savePrediction = async (match) => {
    try {
      ...
      if (isSemiFinalLocked(matches)) {
        setMessage({
          type: 'error',
          text: 'Semi Final predictions are locked'
        });
        return;
      }
      ...
      if (isSemiFinalLocked(freshMatches || [])) {
        setMessage({
          type: 'error',
          text: 'Semi Final predictions are locked'
        });
  ...
  const semiFinalLocked = isSemiFinalLocked(matches);
  const lockTime = getSemiFinalLockTime(matches);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        >
          {semiFinalLocked ? (
            <p className="text-sm font-semibold text-red-300 text-center">
              🔒 Semi Final predictions are locked.
            </p>
          ) : (
            <p className="text-sm text-slate-400 text-center">
              🔒 Semi Final predictions lock 1 hour before the first Semi Final match
              {lockTime && (
                <>
                  {' '}
                  (
        ...
        <div className="mb-8 p-4 bg-slate-900/60 border border-slate-800 rounded-2xl">
          <p className="text-sm text-slate-400 text-center">
            ...
            team to qualify +7 · Exact score (after extra time) +3 · Winning method +1 (only if
            qualified team is correct) · Max 11 points per match
          </p>
        </div>
            ...
            return (
              <div
                ...
                className={`bg-slate-900 border rounded-2xl p-6 ${
                  semiFinalLocked ? 'border-slate-800' : 'border-emerald-900/60'
                }`}
              >
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label
                        htmlFor={`sf_score_a_${match.id}`}
                        className="block text-sm font-semibold text-slate-300 mb-2"
                      >
                        {getTeamFlag(match.team_a)} {match.team_a} Score
                      </label>
                      <input
                        id={`sf_score_a_${match.id}`}
                        type="number"
                        min="0"
                        disabled={semiFinalLocked}
                    ...
                    <div>
                      <label
                        htmlFor={`sf_score_b_${match.id}`}
                        className="block text-sm font-semibold text-slate-300 mb-2"
                      >
                        {getTeamFlag(match.team_b)} {match.team_b} Score
                      </label>
                      <input
                        id={`sf_score_b_${match.id}`}
                        type="number"
                        min="0"
                        disabled={semiFinalLocked}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label
                        htmlFor={`sf_method_${match.id}`}
                        className="block text-sm font-semibold text-slate-300 mb-2"
                      >
                        Winning Method
                      </label>
                      <select
                        id={`sf_method_${match.id}`}
                        disabled={semiFinalLocked}
                    ...
                    <div>
                      <label
                        htmlFor={`sf_qualify_${match.id}`}
                        className="block text-sm font-semibold text-slate-300 mb-2"
                      >
                        Team to Qualify
                      </label>
                      <select
                        id={`sf_qualify_${match.id}`}
                        disabled={semiFinalLocked}
                  ...
                  <button
                    ...
                    disabled={semiFinalLocked || saving[match.id]}
                    className={`w-full py-2.5 rounded-xl font-semibold transition-colors ${
                      semiFinalLocked || saving[match.id]
                        ? 'bg-slate-800 text-slate-500 cursor-not-allowed'
                        : 'bg-emerald-600 text-white hover:bg-emerald-500 active:bg-emerald-700'
                    }`}
                  >
