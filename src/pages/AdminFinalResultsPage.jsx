...
export default function AdminFinalResultsPage() {
  ...
  const calculatePoints = (prediction, result, roundType) => {
    const { winnerPoints, exactScorePoints, methodPoints } = getScoringForRound(roundType);
  ...
  const saveResult = async (match) => {
    try {
      ...
      for (const prediction of predictionsList) {
        const points = calculatePoints(prediction, resultData, match.round_type);
      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-white mb-2">
            👑 Admin Final Stage Results
          </h1>
          <p className="text-slate-400">
            Enter actual Final stage results. Points are recalculated automatically after saving.
        ...
        <div className="mb-8 p-4 bg-slate-900/60 border border-slate-800 rounded-2xl">
          <p className="text-sm text-slate-400 text-center">
            <span className="text-yellow-400 font-semibold">Final:</span> Champion +10 · Exact
            score +5 · Method +1 (only if champion is correct) · Max 16 ·{' '}
            <span className="text-emerald-400 font-semibold">Third Place:</span> Winner +7 ·
            Exact score +3 · Method +1 (only if winner is correct) · Max 11. If the match went to
            penalties, enter the score after extra time and set method to Penalties.
          </p>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mb-6">
                  <div>
                    <label
                      htmlFor={`final_actual_a_${match.id}`}
                      className="block text-sm font-semibold text-slate-300 mb-2"
                    >
                      {getTeamFlag(match.team_a)} {match.team_a} Score
                    </label>
                    <input
                      id={`final_actual_a_${match.id}`}
                      type="number"
                      min="0"
                  ...
                  <div>
                    <label
                      htmlFor={`final_actual_b_${match.id}`}
                      className="block text-sm font-semibold text-slate-300 mb-2"
                    >
                      {getTeamFlag(match.team_b)} {match.team_b} Score
                    </label>
                    <input
                      id={`final_actual_b_${match.id}`}
                      type="number"
                      min="0"
                  ...
                  <div>
                    <label
                      htmlFor={`final_actual_method_${match.id}`}
                      className="block text-sm font-semibold text-slate-300 mb-2"
                    >
                      Winning Method
                    </label>
                    <select
                      id={`final_actual_method_${match.id}`}
                  ...
                  <div>
                    <label
                      htmlFor={`final_actual_qualified_${match.id}`}
                      className="block text-sm font-semibold text-slate-300 mb-2"
                    >
                      {isFinal ? 'Champion' : 'Winner'}
                    </label>
                    <select
                      id={`final_actual_qualified_${match.id}`}
                  ...
                  <div className="mb-6 p-4 bg-slate-800/60 rounded-xl border border-slate-700">
                    ...
                    <p className="text-sm text-slate-300">
                      <span className="font-bold text-white">
                        ...
                        - {match.actual_team_b_score} {getTeamFlag(match.team_b)}{' '}
                        {match.team_b}
                      </span>{' '}
                      · Method:{' '}
